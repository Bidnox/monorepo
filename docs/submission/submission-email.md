Subject: Cleanverse Build submission — Bidnox

Hi Cleanverse team,

Please find our submission for the DeFi track: **Bidnox, compliant confidential receivables financing**.

- Public GitHub: [add URL]
- Live frontend: [add URL]
- Demo video: [add URL]
- One-page summary: `docs/submission/one-page-summary.md`
- Deployed chain: Base Sepolia (`84532`)
- ComplianceGate: `0x12badb8fd1828AB70Ea5FD4F5142Bc8c9e8f537d`
- ReceivableRegistry: `0xCad5d39Dc42757969323608a9207B283dbDE3b37`
- ConfidentialAuction: `0xDA6F7Fe360f7700d6E0d867bDC7f51C048E33c82`
- Cleanverse aUSDC: `0xaC0893567D43C3E7e6e35a72803df05416C1f20D`

Our live Base Sepolia demo completes seller creation, buyer confirmation, two confidential bids, winner reveal, real aUSDC financing, and real aUSDC repayment. Cleanverse A-Pass/aUSDC eligibility is enforced through short-lived, action-bound onchain permits.

Regards,
Bidnox
